$(document).ready(function () {
	...
	if (location.href.match(/ingredients/)) {
		//if you are currently viewing the ingredients page
		...
		//if you are currently viewing the recipes page
		getNames("/get_names", "ingredient", "name");
		...
		$("input#ingredient_name").autocomplete({
			...
			onAutocomplete: function () {
				...
				matchedIngredient = true;
				Materialize.toast(
					"<i class='material-icons positive'>check_circle</i>Found ingredient",
					3000
				);
			},
			minLength: 1, // The minimum length of the input for the autocomplete to start. Default: 1.
		...
		//if you are currently viewing the home (index) page
		$("#meals-list li").each(function () {
			//update the nutritional data for each recipe in meals list
	...
	function deleteItem(table, column, value) {
		...
		$.ajax({
			...
			success: function () {
				if (table == "ingredient") {
					Materialize.toast(
						"<i class='material-icons negative'>delete_forever</i>Deleted ingredient",
						3000
					);
					getNames("/get_names", "ingredient", "name");
				} else if (table == "recipe") {
					Materialize.toast(
						"<i class='material-icons negative'>delete_forever</i>Deleted recipe",
						3000
					);
					getNames("/get_names", "recipe", "name");
				}
	...
	function saveRecipe() {
		...
		$("#ingredient-list li").each(function () {
			//process all the ingredients in the ingredients list
		...
		$("#step-list li").each(function () {
			//process all the steps in the steps list
		...
		if (isFormValid) {
			...
			$.ajax({
				...
				success: function (result) {
					if (result == "saved recipe") {
						if (action == "save") {
							Materialize.toast(
								"<i class='material-icons positive'>cloud_done</i>Saved recipe",
								3000
							);
						} else if (action == "update") {
							Materialize.toast(
								"<i class='material-icons positive'>cloud_done</i>Updated recipe",
								3000
							);
						}
			...
			Materialize.toast(
				"<i class='material-icons neutral'>warning</i>All fields must be filled out",
				3000
			);
			$(window).scrollTop(0); //scroll window to top
		}
			...
			success: function (result) {
				if (result) {
					...
					if ($("#ingredient-list li").length == 0) {
						//if zero ingredients in list (stops ingredients being added every keystroke)
					...
					if ($("#step-list li").length == 0) {
						//if zero steps in list (stops steps being added every keystroke)
					...
					Materialize.toast(
						"<i class='material-icons positive'>check_circle</i>Loaded recipe data",
						3000
					);
				} else {
		...
		if (isFormValid) {
			$.ajax({
				...
				success: function () {
					if (action == "save") {
						Materialize.toast(
							"<i class='material-icons positive'>cloud_done</i>Saved nutritional data",
							3000
						);
					} else if (action == "update") {
						Materialize.toast(
							"<i class='material-icons positive'>cloud_done</i>Updated nutritional data",
							3000
						);
					}
	...
	$("#delete-nutrition-data").on("click", function (event) {
		...
		if (matchedIngredient) {
			...
			Materialize.toast(
				"<i class='material-icons neutral'>warning</i>Ingredient doesn't exist",
				3000
			);
			$(window).scrollTop(0); //scroll window to top
		}
	...
	$("#delete-recipe").on("click", function (event) {
		...
		if (matchedRecipe) {
			...
			Materialize.toast(
				"<i class='material-icons neutral'>warning</i>Recipe doesn't exist",
				3000
			);
			$(window).scrollTop(0); //scroll window to top
		}
	...
	$("#add-step-to-recipe").on("click", function (event) {
		...
		if (
			step_number.length > 0 &&
			step_description.length > 0 &&
			step_description.length < 145
		) {
			//if step number and description are filled in
			addStepToRecipe(step_number, step_description);
			updateRecipeStatus();
		} else {
			if (step_description.length > 144) {
				Materialize.toast(
					"<i class='material-icons neutral'>warning</i>Step description is too long. Must be less than 145 characters.",
					4000
				);
			} else {
				Materialize.toast(
					"<i class='material-icons neutral'>warning</i>Please fill out all step fields",
					3000
				);
			}
		}
	...
	function addStepToRecipe(step_number, step_description) {
		...
		Materialize.toast(
			"<i class='material-icons positive'>check_circle</i>Added to step list",
			3000
		);
	}
	$("#add-to-meal-plan").on("click", function (event) {
		...
		if (recipe_name.length > 0) {
			...
			if (matchedRecipe) {
				...
				Materialize.toast(
					`<i class='material-icons neutral'>warning</i><span>Recipe not found</span>
                <form action="/recipes" method='POST'><button class="btn-flat toast-action recipe-toast" name="recipe_name" value="${$(
					"#recipe_name"
				).val()}">CREATE RECIPE</button></form>`,
					10000
				);
			}
		} else {
			Materialize.toast(
				"<i class='material-icons neutral'>warning</i>Please fill out recipe name",
				3000
			);
		}
	});
	function updateHomeStatus() {
		...
		$("#meals-list li").each(function () {
			//updates the meal object using recipes from our meals list
			meal.recipe_name_list.push($(this).find("span").text().trim());
			meal.meal_type_list.push($(this).find("div:eq(1)").text().trim());
		});
		if (isFormValid) {
			$.ajax({
	...
	function addRecipeToMealPlan(recipe_name, meal_type) {
		...
		$("#recipe_name").val(""); //reset recipe name
		Materialize.toast(
			"<i class='material-icons positive'>check_circle</i>Added to meal list",
			3000
		);
	}
			...
			success: function (result) {
				if (result) {
					...
					if (operation == "add") {
						...
						$("#energy").text(
							parseInt($("#energy").text()) + result.energy
						); //set total energy as current value + new added value
					} else {
						total = parseInt($("#energy").text()) - result.energy;
						$("#energy").text(
							parseInt($("#energy").text()) - result.energy
					...
					if (percent > 100) {
						//prevent percentage going over 100%
						percent = 100;
					}
					...
					if (operation == "add") {
						total =
							parseInt($("#carbohydrate").text()) +
							result.carbohydrate;
						$("#carbohydrate").text(
							parseInt($("#carbohydrate").text()) +
								result.carbohydrate
						); //set total carbohydrate as current value + new added value
					} else {
						total =
							parseInt($("#carbohydrate").text()) -
							result.carbohydrate;
					...
					$("#carbohydrate_bar .determinate").css({
						width: percent + "%",
					});
					...
					if (operation == "add") {
						...
						$("#fats").text(
							parseInt($("#fats").text()) + result.fats
						); //set total fats as current value + new added value
					} else {
						total = parseInt($("#fats").text()) - result.fats;
						$("#fats").text(
							parseInt($("#fats").text()) - result.fats
					...
					if (operation == "add") {
						...
						$("#protein").text(
							parseInt($("#protein").text()) + result.protein
						); //set total protein as current value + new added value
					} else {
						total = parseInt($("#protein").text()) - result.protein;
						$("#protein").text(
							parseInt($("#protein").text()) - result.protein
					...
					if (operation == "add") {
						...
						$("#calcium").text(
							parseInt($("#calcium").text()) + result.calcium
						); //set total calcium as current value + new added value
					} else {
						total = parseInt($("#calcium").text()) - result.calcium;
						$("#calcium").text(
							parseInt($("#calcium").text()) - result.calcium
					...
					if (operation == "add") {
						...
						$("#iron").text(
							parseInt($("#iron").text()) + result.iron
						); //set total iron as current value + new added value
					} else {
						total = parseInt($("#iron").text()) - result.iron;
						$("#iron").text(
							parseInt($("#iron").text()) - result.iron
					...
					if (operation == "add") {
						...
						$("#zinc").text(
							parseInt($("#zinc").text()) + result.zinc
						); //set total zinc as current value + new added value
					} else {
						total = parseInt($("#zinc").text()) - result.zinc;
						$("#zinc").text(
							parseInt($("#zinc").text()) - result.zinc
	...
	$("#add-ingredient-to-recipe").on("click", function (event) {
		...
		if (value.length > 0 && amount.length > 0) {
			//if ingredient name and amount exist
			if (matchedIngredient) {
				addIngredientToRecipe(value, amount, unit);
				updateRecipeStatus();
			} else {
				Materialize.toast(
					`<i class='material-icons neutral'>warning</i><span>Ingredient not found</span>
                <form action="/ingredients" method="POST"><button class="btn-flat toast-action ingredient-toast" name="ingredient_name" value="${$(
					"#ingredient_name"
				).val()}">CREATE INGREDIENT</button></form>`,
					10000
				);
			}
		} else {
			Materialize.toast(
				"<i class='material-icons neutral'>warning</i>Please fill out all ingredient fields",
				3000
			);
		}
	});
	...
	function addIngredientToRecipe(value, amount, unit) {
		...
		$("#ingredient_units").material_select(); //re-initialized
		Materialize.toast(
			"<i class='material-icons positive'>check_circle</i>Added to ingredient list",
			3000
		);
	}
		...
		$.ajax({
			...
			success: function (result) {
				if (result) {
					...
					if (location.href.match(/ingredients/)) {
						$("#ingredient_amount").val(
							result[0].ingredient_amount
						);
						Materialize.toast(
							"<i class='material-icons positive'>check_circle</i>Loaded nutritional data",
							3000
						);
					} else {
						$("#ingredient_amount").val();
